...
import axios, { endpoints } from '../API'; // Import axios và endpoints
...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post(endpoints.login, {
        username: email, // Sử dụng email làm username theo như API đăng ký của bạn
        password: password,
      });
      if (response.status === 200) {
        ...
        try {
          ...
          navigation.navigate('Main'); // Chuyển đến route 'Main'
        } catch (error) {
          console.error('Lỗi khi lưu token:', error);
          setErrorMessage('Lỗi khi lưu thông tin đăng nhập.');
        }
      } else {
        console.error('Lỗi đăng nhập:', response.data);
        setErrorMessage(response.data.error || 'Đăng nhập thất bại. Vui lòng kiểm tra email và mật khẩu.');
      }
    } catch (error) {
  ...
  const handleRegister = () => {
    ...
    navigation.navigate('Register'); // Chuyển đến route 'Register'
  };
...
const styles = StyleSheet.create({
  ...
  bottomNav: {
    ...
    borderTopWidth: 1,
    borderColor: '#ddd',
    // Add styles for your bottom navigation items here
  },
  error: {
    color: 'red',
    marginBottom: 10,
